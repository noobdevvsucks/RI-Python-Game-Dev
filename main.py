...
class Enemy:
    def __init__(self, x, y):
        ...
        self.jitter = pygame.Vector2(
            uniform(-ENEMY_JITTER, ENEMY_JITTER),
            uniform(-ENEMY_JITTER, ENEMY_JITTER)
        )
        self.jitter_timer = randint(30, 70)
        self.speed_timer = randint(120, 300)
        self.rect = pygame.Rect(int(x), int(y), ENEMY_SIZE, ENEMY_SIZE)
    ...
    def update(self, player_position):
        ...
        if self.jitter_timer <= 0:
            self.jitter.x = uniform(-ENEMY_JITTER, ENEMY_JITTER)
            self.jitter.y = uniform(-ENEMY_JITTER, ENEMY_JITTER)
            self.jitter_timer = randint(30, 70)
        ...
        self.velocity = self.velocity.lerp(target_velocity, ENEMY_ACCELERATION)
...
class Game:
    def __init__(self):
        ...
        self.player_position = pygame.Vector2(100, 100)
    ...
    def reset(self):
        self.player_position.update(100, 100)
    ...
    def add_enemy(self, x=None, y=None):
        if x is None:
            x = SCREEN_WIDTH // 2 - ENEMY_SIZE // 2
        if y is None:
            y = SCREEN_HEIGHT // 2 - ENEMY_SIZE // 2
    ...
    def create_beam(self):
        self.beam_angle = uniform(0, math.pi)
        ...
        length = math.hypot(SCREEN_WIDTH, SCREEN_HEIGHT) * 2
        self.beam_start = center - direction * length
        self.beam_end = center + direction * length
    ...
    def start_beam_wave(self):
        ...
        self.beam_number = 1
        self.beam_phase = "warning"
        self.beam_timer = BEAM_WARNING_DURATION
        self.create_beam()
    ...
    def beam_hits_player(self):
        if self.beam_start is None or self.beam_end is None:
            return False

        beam_length = math.hypot(SCREEN_WIDTH, SCREEN_HEIGHT) * 2

        beam_surface = pygame.Surface(
            (int(beam_length), BEAM_WIDTH),
            pygame.SRCALPHA
        )

        beam_surface.fill((255, 0, 0, 255))

        rotated_beam = pygame.transform.rotate(
            beam_surface,
            -math.degrees(self.beam_angle)
        )
        ...
        beam_mask = pygame.mask.from_surface(rotated_beam)
        ...
        return beam_mask.overlap(player_mask, offset) is not None
    ...
    def update_beam(self, dt):
        ...
        if self.beam_active and self.beam_phase == "beam":
    ...
    def get_screen_shake(self):
        ...
        strength = SCREEN_SHAKE_AMOUNT * (
            self.screen_shake_timer / SCREEN_SHAKE_DURATION
        )

        return (
            randint(-int(strength), int(strength)),
            randint(-int(strength), int(strength))
        )
    ...
    def teleport(self):
        if self.teleport_available and self.teleport_marker is None:
            self.teleport_marker = pygame.Vector2(self.player_position)
            self.teleport_available = False
            return

        if self.teleport_marker is not None:
            self.player_position.update(self.teleport_marker)
            self.player_velocity.update(0, 0)
            self.teleport_marker = None
    ...
    def update_player(self):
        ...
        elif self.player_position.x > SCREEN_WIDTH - PLAYER_SIZE:
            self.player_position.x = SCREEN_WIDTH - PLAYER_SIZE
            self.player_velocity.x = 0
        ...
        elif self.player_position.y > SCREEN_HEIGHT - PLAYER_SIZE:
            self.player_position.y = SCREEN_HEIGHT - PLAYER_SIZE
            self.player_velocity.y = 0
    ...
    def update_enemy_count(self):
        desired_count = 2 + self.score // ENEMY_SCORE_INTERVAL

        # ---------------------------------------------------------------------
        # START LASER WAVE AT EVERY 10-ENEMY THRESHOLD
        # ---------------------------------------------------------------------

        reached_waves = desired_count // 10
    ...
    def update(self, dt):
        ...
        for enemy in self.enemies:
            enemy.update(self.player_position)
    ...
    def centered_text(self, text, text_font, colour, y):
        surface = text_font.render(text, True, colour)
    ...
    def draw_menu(self):
        self.centered_text("ARENA", big_font, WHITE, 120)
        self.centered_text(
